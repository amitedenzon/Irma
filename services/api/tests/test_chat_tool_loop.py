...
def _build_app(
    ...
    *,
    ...
    app.state.llm = llm
    if registry is None:
        registry = {getattr(llm, "backend", "fake"): llm} if llm is not None else {}
        default = default or (next(iter(registry)) if registry else None)
...
async def test_503_when_llm_missing() -> None:
    ...
    app.state.default_backend = None
    app.state.bus = StateBus()
...
class _StatefulFakeLLM:
    """Mimics claude_cli — stateful, no tool loop."""
    ...
    async def complete(self, **kwargs: Any) -> Any:
        self.calls.append(kwargs)
        return TextResult(text=f"stateful:{kwargs.get('session_id')}")


@pytest.mark.asyncio
async def test_get_backends_lists_registry() -> None:
    primary = _FakeLLM(TextResult(text="x"))
    secondary = _StatefulFakeLLM()
    app = _build_app(
        primary,
        [_EchoTool()],
        registry={"fake": primary, "claude_cli": secondary},
        default="fake",
    )
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get("/api/v1/chat/backends")
    assert resp.status_code == 200
    body = resp.json()
    assert body["default"] == "fake"
    assert sorted(body["available"]) == ["claude_cli", "fake"]
    assert body["models"]["claude_cli"] == "claude-opus-4-7"


@pytest.mark.asyncio
async def test_unknown_backend_returns_400() -> None:
    llm = _FakeLLM(TextResult(text="x"))
    app = _build_app(llm, [_EchoTool()])
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post(
...
@pytest.mark.asyncio
async def test_claude_cli_requires_session_id() -> None:
    fake = _StatefulFakeLLM()
    app = _build_app(
        fake,
        [_EchoTool()],
        registry={"claude_cli": fake},
        default="claude_cli",
    )
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
...
@pytest.mark.asyncio
async def test_claude_cli_validates_uuid_format() -> None:
    fake = _StatefulFakeLLM()
    app = _build_app(
        fake,
        [_EchoTool()],
...
@pytest.mark.asyncio
async def test_claude_cli_dispatches_with_session_id_and_skips_tools() -> None:
    primary = _FakeLLM(TextResult(text="from-default"))
    stateful = _StatefulFakeLLM()
    app = _build_app(
        primary,
        [_EchoTool()],
        registry={"fake": primary, "claude_cli": stateful},
        default="fake",
    )
    transport = ASGITransport(app=app)
    ...
    assert resp.status_code == 200
    body = resp.json()
    assert body["backend"] == "claude_cli"
    assert body["reply"] == f"stateful:{sid}"
    # Default backend should not have been called at all.
    assert primary.calls == []
    # Stateful backend received the session_id and no tools.
    assert stateful.calls[0]["session_id"] == sid
    assert "tools" not in stateful.calls[0] or stateful.calls[0]["tools"] is None


@pytest.mark.asyncio
async def test_default_backend_used_when_request_omits_backend() -> None:
    primary = _FakeLLM(TextResult(text="ok"))
    other = _FakeLLM(TextResult(text="wrong"))
    app = _build_app(
        primary,
        [_EchoTool()],
        registry={"fake": primary, "other": other},
        default="fake",
    )
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
